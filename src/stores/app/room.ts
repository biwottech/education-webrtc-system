...
import {
  ...
  EduTextMessage,
  EduSceneType
} from "./../../sdk/education/interfaces/index";
...
import {
  LocalUserRenderer,
  UserRenderer
} from "../../sdk/education/core/media-service/renderer/index";
...
} from "@/sdk/education/core/media-service/interfaces";
...
import Artc from "agora-rtc-sdk";
...
export class RoomStore extends SimpleInterval {
  ...
  get cameraId(): string {
    ...
    const idx = this.cameraList.findIndex(
      (it: any) => it.label === this.cameraLabel
    );
  ...
  get microphoneId(): string {
    const defaultValue = "";
    const idx = this.microphoneList.findIndex(
      (it: any) => it.label === this.microphoneLabel
    );
        ...
        .catch((err) => {
          ...
          if (err.code === "ELECTRON_PERMISSION_DENIED") {
            this.appStore.uiStore.addToast(
              t("toast.failed_to_enable_screen_sharing_permission_denied")
            );
          } else {
            this.appStore.uiStore.addToast(
              t("toast.failed_to_enable_screen_sharing") +
  ...
  async startNativeScreenShareBy(windowId: number) {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.failed_to_initiate_screen_sharing") + `${err.msg}`
      );
    }
  }
  ...
  async playAudioMixing() {
    ...
    const audioMixingTrack: IBufferSourceAudioTrack = await AgoraRTC.createBufferSourceAudioTrack(
      options
    );
  ...
  @observable
  activeAudioEffect: string = "";

  @observable
  activeAudioEffectSoundId: number = 0;

  _localStream: Artc.Stream | undefined;

  _client: Artc.Client | undefined;

  @action
  async playAudioEffect(filePath: string) {
    // https://github.com/AgoraIO/Advanced-Audio/blob/master/Web/AgoraAudioIO-Web-Webpack/src/rtc-client.js
    const teacherStream = this.teacherStream;
    const studentStreams = this.studentStreams;
    const client = Artc.createClient({ mode: "rtc", codec: "vp8" });
    client.init(process.env.REACT_APP_AGORA_APP_ID as string, () => {
      console.log("init success");
    });

    const localStream = Artc.createStream({
      streamID: teacherStream.streamUuid,
      audio: true,
      video: true,
      screen: false
      // screen: false,
      // microphoneId: teacherStream.,
      // cameraId: data.cameraId
    });

    const soundId = Date.now() % 10000;

    localStream.init(
      () => {
        console.log("init local stream success");
        // play stream with html element id "local_stream"
        // localStream.play("local_stream", { fit: "cover" });
        localStream.playEffect(
          {
            soundId, // id of the specified audio effect
            cycle: 1, // playback loops,
            filePath
          },
          (err) => {
            if (err) {
              console.log("failed to play effec!!");
            }
          }
        );
      },
    ...
    this._client = client;
    this.activeAudioEffect = filePath;
    this.activeAudioEffectSoundId = soundId;
  }

  @action
  async toggleAudioEffect(play?: boolean) {
    if (!this._localStream) {
      console.log("this._localStream is undefined");
      return;
    }

    if (play) {
      this._localStream.resumeAllEffects((err) => {
        if (err) {
          console.log(err);
        }
      });
    } else {
      this._localStream.pauseAllEffects((err) => {
        console.log(err);
      });
    }
  }
  ...
  async stopAudioEffect() {
    ...
    this._localStream.stopAllEffects((err) => {
      console.log(err);
    });
  }
  ...
  async closeMicrophone() {
    if (this.microphoneLock)
      return console.warn("[demo] closeMicrophone microphone is locking");
  ...
  async stopWebSharing() {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.failed_to_end_screen_sharing") + `${err.msg}`
      );
    } finally {
      this.waitingShare = false;
    }
  ...
  async startWebSharing() {
    try {
      ...
      if (this.mediaService.screenRenderer) {
        ...
        this._screenVideoRenderer = undefined;
        this.appStore.uiStore.addToast(
          t("toast.failed_to_initiate_screen_sharing_to_remote") + `${err.msg}`
        );
      } else {
  ...
  async joinRTC(args: any) {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.failed_to_join_rtc_please_refresh_and_try_again")
      );
      console.warn(err);
      throw err;
    }
  ...
  async join() {
    try {
          ...
          if (evt.type === "main") {
            ...
            if (localStream && localStream.state !== 0) {
              ...
              console.log(
                ...
                this.joiningRTC,
                " _eduStream",
                JSON.stringify(this._cameraEduStream)
              );
              if (this.joiningRTC) {
                if (this._hasCamera) {
            ...
            if (msg) {
              ...
              if (type === PeerInviteEnum.teacherStop) {
                try {
                  ...
                  await this.closeMicrophone();
                  this.appStore.uiStore.addToast(
                    t("toast.co_video_close_success")
                  );
                } catch (err) {
                  this.appStore.uiStore.addToast(
                    t("toast.co_video_close_failed")
                  );
                  console.warn(err);
                }
              }
      ...
      if (this.roomInfo.userRole === "teacher") {
        ...
        const sceneType =
          +this.roomInfo.roomType === 2
            ? EduSceneType.SceneLarge
            : +this.roomInfo.roomType;
        const userRole =
          sceneType === EduSceneType.SceneLarge ? "audience" : "broadcaster";
        await roomManager.join({
          userRole: userRole,
          roomUuid,
          userName: `${this.roomInfo.userName}`,
      ...
      this.appStore._boardService = new EduBoardService(
        roomManager.userToken,
        roomManager.roomUuid
      );
      ...
      let canPublish =
        this.roomInfo.userRole === "teacher" ||
      ...
      if (canPublish) {
        ...
        this.appStore.uiStore.addToast(
          t("toast.publish_business_flow_successfully")
        );
        ...
        try {
          ...
          this.appStore.uiStore.addToast(
            t("toast.media_method_call_failed") + `: ${err.msg}`
          );
          console.warn(err);
        }
      }
  ...
  get teacherStream(): EduMediaStream {
    ...
    if (localUser && localUser.userRole === "teacher" && this.cameraEduStream) {
      return {
        local: true,
  ...
  private getStreamBy(userUuid: string): EduStream {
    return this.streamList.find(
      (it: EduStream) => (it.userInfo.userUuid as string) === userUuid
    ) as EduStream;
  }
  ...
  async leave() {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.successfully_left_the_business_channel")
      );
      this.delInterval("timer");
      this.reset();
      this.resetRoomInfo();
  ...
  async startClass() {
    try {
      await this.roomManager?.userService.updateCourseState(
        EduCourseState.EduCourseStateStart
      );
  ...
  async stopClass() {
    try {
      await this.roomManager?.userService.updateCourseState(
        EduCourseState.EduCourseStateStop
      );
  ...
  async muteAudio(userUuid: string, isLocal: boolean) {
    if (isLocal) {
      ...
      await this.roomManager?.userService.remoteStopStudentMicrophone(
        targetStream as EduStream
      );
    }
  }
  ...
  async unmuteAudio(userUuid: string, isLocal: boolean) {
    ...
    if (isLocal) {
      ...
      await this.roomManager?.userService.remoteStartStudentMicrophone(
        targetStream as EduStream
      );
    }
  }
  ...
  async muteVideo(userUuid: string, isLocal: boolean) {
    ...
    if (isLocal) {
      ...
      await this.roomManager?.userService.remoteStopStudentCamera(
        targetStream as EduStream
      );
    }
  }
  ...
  async unmuteVideo(userUuid: string, isLocal: boolean) {
    ...
    if (isLocal) {
      ...
      await this.roomManager?.userService.remoteStartStudentCamera(
        targetStream as EduStream
      );
    }
  }
  ...
  async startRecording() {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.start_recording_failed") + `, ${err.msg}`
      );
    }
  }
  ...
  async stopRecording() {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.failed_to_end_recording") + `, ${err.msg}`
      );
    }
  }
  ...
  async callApply() {
    try {
      const teacher = this.roomManager
        ?.getFullUserList()
        .find((it: EduUser) => it.userUuid === this.teacherStream.userUuid);
      if (teacher) {
        await this.roomManager?.userService.sendCoVideoApply(teacher);
      }
    } catch (err) {
      this.appStore.uiStore.addToast(
        t("toast.failed_to_initiate_a_raise_of_hand_application") +
          ` ${err.msg}`
      );
    }
  }
  ...
  async callEnded() {
    try {
      ...
      this.appStore.uiStore.addToast(
        t("toast.failed_to_end_the_call") + ` ${err.msg}`
      );
    }
  }

  showDialog(userName: string, userUuid: any) {
    const isExists = this.appStore.uiStore.dialogs
      .filter((it: DialogType) => it.dialog.userUuid)
  ...
  removeDialogBy(userUuid: any) {
    const target = this.appStore.uiStore.dialogs
      .filter((it: DialogType) => it.dialog.userUuid)
      .find((it: DialogType) => it.dialog.userUuid === userUuid);
    if (target) {
  ...
  async teacherRejectApply() {
    ...
    const user = this.roomManager
      ?.getFullUserList()
      .find((it) => it.userUuid === userUuid);
    if (user) {
  ...
  async teacherAcceptApply() {
    const userUuid = (this.notice as any).userUuid;
    const user = this.roomManager?.data.userList.find(
      (it) => it.user.userUuid === userUuid
    );
    if (user) {
